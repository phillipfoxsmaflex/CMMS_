...
export const getAllDocuments = (): AppThunk =>
  async (dispatch) => {
    try {
      dispatch(slice.actions.setLoadingGet({ loading: true }));
      console.log('[DEBUG] Fetching all documents');
      const documents = await api.get<Document[]>('documents/all');
      console.log('[DEBUG] All documents received:', documents);
      const key = 'ALL-0';
      dispatch(slice.actions.getDocuments({ key, documents }));
      console.log('[DEBUG] All documents dispatched to store');
    } catch (error) {
      console.error('[ERROR] Error fetching all documents:', error);
      console.error('[ERROR] Error details:', JSON.stringify(error));
    } finally {
      dispatch(slice.actions.setLoadingGet({ loading: false }));
    }
...
export const downloadBatch = async (documentIds: number[]): Promise<void> => {
  try {
    const response = await api.post<Response>('documents/download-batch', documentIds, {
      raw: true
    });
...
export const getPreviewUrl = (documentId: number): string => {
  // Returns the preview URL for image documents
