...
function Files() {
  ...
  useEffect(() => {
    if (hasViewPermission(PermissionEntity.DOCUMENTS)) dispatch(getFiles(criteria));
  }, [criteria]);
  ...
  const columns: GridEnrichedColDef[] = [
    ...
    },
    {
      ...
      getActions: (params: GridRowParams<File>) => {
        ...
        if (!hasEditPermission(PermissionEntity.DOCUMENTS, params.row)) {
          actions.shift();
        }
        if (!hasDeletePermission(PermissionEntity.DOCUMENTS, params.row)) {
          actions.pop();
        }
        return actions;
      }
  ...
  if (hasFeature(PlanFeature.FILE)) {
    if (hasViewPermission(PermissionEntity.DOCUMENTS))
      return (
        <>
          <Helmet>
            <title>{t('files')}</title>
