        ...
        <Box>
          <Form
            ...
            onSubmit={async (values) => {
              ...
              try {
                // Extract files and image before creating asset
                ...
                const { files, image, ...assetData } = formattedValues;
                
                // Create asset first - use API directly to get the response
                const createdAsset = await api.post<AssetDTO>('assets', assetData);
                
                // Update Redux state
                await dispatch(getAssets(criteria));
                
                // Upload documents if there are any files
                const allFilesToUpload = [...filesToUpload, ...imagesToUpload];
                if (allFilesToUpload.length > 0 && createdAsset?.id) {
                  await dispatch(uploadDocuments('ASSET', createdAsset.id, allFilesToUpload));
                }
                
                await onCreationSuccess();
                
                // Refresh asset hierarchy
