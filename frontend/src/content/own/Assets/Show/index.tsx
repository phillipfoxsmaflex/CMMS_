...
import AssetDashboardSettings from '../../AssetMonitoring/AssetDashboardSettings';
...
const ShowAsset = ({}: PropsType) => {
  ...
  const tabs = [
    ...
    { value: 'dashboard', label: t('dashboard_settings') }
  ];
          ...
          <Form
            ...
            onSubmit={async (values) => {
              ...
              try {
                ...
                // Update asset (without files)
                await dispatch(editAsset(Number(assetId), assetData));
                
                // Upload new documents if any
                const allNewFilesToUpload = [...newFiles, ...newImages];
                if (allNewFilesToUpload.length > 0) {
                  await dispatch(uploadDocuments('ASSET', Number(assetId), allNewFilesToUpload));
                }
      ...
      >
        {isNumeric(assetId) ? (
          tabIndex === 0 ? (
            ...
            tabIndex === 7 && <AssetDashboardSettings asset={asset} />
          )
        ) : null}
        <ConfirmDialog
          open={openDelete}
