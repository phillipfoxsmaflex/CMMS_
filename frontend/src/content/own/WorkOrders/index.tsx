        ...
        <Box>
          <Form
            ...
            onSubmit={async (values) => {
              ...
              try {
                // Extract files and image before creating work order
                ...
                const { files, image, ...workOrderData } = formattedValues;
                
                // Create work order first - use API directly to get the response
                const createdWorkOrder = await api.post<WorkOrder>('work-orders', workOrderData);
                
                // Update Redux state
                await dispatch(getWorkOrders(criteria));
                
                // Upload documents if there are any files
                const allFilesToUpload = [...filesToUpload, ...imagesToUpload];
                if (allFilesToUpload.length > 0 && createdWorkOrder?.id) {
          ...
          <Form
            ...
            onSubmit={async (values) => {
              ...
              try {
                ...
                await dispatch(
                  editWorkOrder(currentWorkOrder?.id, workOrderData)
                );

                // Upload new documents if any
                const allNewFilesToUpload = [...newFiles, ...newImages];
                if (allNewFilesToUpload.length > 0) {
                  await dispatch(uploadDocuments('WORK_ORDER', currentWorkOrder.id, allNewFilesToUpload));
                }
