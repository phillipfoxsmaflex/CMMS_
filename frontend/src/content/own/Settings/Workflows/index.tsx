...
function Workflows() {
  ...
  > = {
    ...
    ALERT_NAME_IS: { type: 'text', accessor: 'alertName' },
    SEVERITY_IS: {
      type: 'select',
    ...
    if (
      ...
      if (
        ...
        const getTypeAccessor = () => {
          ...
          if (currentMainCondition === 'WEBHOOK') return 'workOrder';
        };
        ...
        const workflow = {
          ...
          secondaryConditions: currentConditions.map((condition) => {
            ...
            // For WEBHOOK, don't add the type-based condition field
            if (currentMainCondition !== 'WEBHOOK') {
              result[`${getTypeAccessor()}Condition`] = condition.type;
            }
            
            result[config.accessor] = formattedValue;
