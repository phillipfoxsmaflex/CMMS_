  ...
  const allPermissions: {
    ...
    delete: [
      ...
      },
      {
        label: t('files'),
        condition: (role: Role) =>
          role.deleteOtherPermissions.includes(PermissionEntity.DOCUMENTS)
      },
      {
        label: t('categories'),
        condition: (role: Role) =>
  ...
  return (
    ...
    >
      ...
      <Grid item xs={12}>
        ...
        <Box>
          <PermissionMatrix
            viewPermissions={role.viewPermissions || role.viewOtherPermissions || []}
            createPermissions={role.createPermissions || []}
            editPermissions={role.editPermissions || role.editOtherPermissions || []}
            deletePermissions={role.deletePermissions || role.deleteOtherPermissions || []}
            onPermissionChange={() => {}} // Read-only in detail view
            disabled={true}
          />
        </Box>
      </Grid>
