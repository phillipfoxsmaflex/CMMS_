        ...
        <Box>
          <Form
            ...
            onSubmit={async (values) => {
              ...
              try {
                // Extract files and image before creating request
                ...
                const { files, image, ...requestData } = formattedValues;
                
                // Create request first - use API directly to get the response
                const createdRequest = await api.post<Request>('requests', requestData);
                
                // Update Redux state
                await dispatch(getRequests(criteria));
                
                // Upload documents if there are any files
                const allFilesToUpload = [...filesToUpload, ...imagesToUpload];
                if (allFilesToUpload.length > 0 && createdRequest?.id) {
                  await dispatch(uploadDocuments('REQUEST', createdRequest.id, allFilesToUpload));
                }
          ...
          <Form
            ...
            onSubmit={async (values) => {
              ...
              try {
                ...
                await dispatch(editRequest(currentRequest?.id, requestData));
                
                // Upload new documents if any
                const allNewFilesToUpload = [...newFiles, ...newImages];
                if (allNewFilesToUpload.length > 0) {
                  await dispatch(uploadDocuments('REQUEST', currentRequest.id, allNewFilesToUpload));
                }
