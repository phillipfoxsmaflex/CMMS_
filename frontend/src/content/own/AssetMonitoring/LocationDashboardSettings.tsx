...
interface LocationDashboardSettingsProps {
  location: Location;
  onUpdate?: () => void;
}

function LocationDashboardSettings({ location, onUpdate }: LocationDashboardSettingsProps) {
  ...
  useEffect(() => {
    if (location.dashboardUrl) {
      setDashboardUrl(location.dashboardUrl);
    }
  }, [location]);
  ...
  const handleSave = async () => {
    setLoading(true);
    try {
      await api.post(`/locations/${location.id}/dashboard`, {
        url: dashboardUrl,
        config: null
      });
...
export default LocationDashboardSettings;