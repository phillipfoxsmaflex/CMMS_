        ...
        <Box>
          <Form
            ...
            onSubmit={async (values) => {
              ...
              try {
                // Extract files and image before creating location
                const filesToUpload = formattedValues.files || [];
                const imagesToUpload = formattedValues.image || [];
                
                // Remove files/image from payload - will upload after creation
                const { files, image, ...locationData } = formattedValues;
                
                // Create location first - use API directly to get the response
                const createdLocation = await api.post<Location>('locations', locationData);
                
                // Update Redux state
                await dispatch(getLocations());
                
                // Upload documents if there are any files
                const allFilesToUpload = [...filesToUpload, ...imagesToUpload];
                if (allFilesToUpload.length > 0 && createdLocation?.id) {
                  await dispatch(uploadDocuments('LOCATION', createdLocation.id, allFilesToUpload));
                }
                
                await onCreationSuccess();
                
                // Refresh location hierarchy
        ...
        <Box>
          <Form
            ...
            onSubmit={async (values) => {
              ...
              try {
                // Extract new files (files without .id are new uploads)
                const newFiles = formattedValues.files?.filter((file) => !file.id) || [];
                const newImages = formattedValues.image || [];
                
                // Remove files/image from payload - will upload after edit
                const { files, image, ...locationData } = formattedValues;
                
                // Update location (without files)
                await dispatch(editLocation(currentLocation.id, locationData));
                
                // Upload new documents if any
                const allNewFilesToUpload = [...newFiles, ...newImages];
                if (allNewFilesToUpload.length > 0) {
                  await dispatch(uploadDocuments('LOCATION', currentLocation.id, allNewFilesToUpload));
                }
