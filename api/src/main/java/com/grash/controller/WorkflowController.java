...
public class WorkflowController {
    ...
    public Workflow create(@ApiParam("Workflow") @Valid @RequestBody WorkflowPostDTO workflowReq, HttpServletRequest req) {
        ...
        if (user.getRole().getViewPermissions().contains(PermissionEntity.SETTINGS)) {
            ...
            if (user.getCompany().getSubscription().getSubscriptionPlan().getFeatures().contains(PlanFeatures.WORKFLOW) || workflowsCount == 0) {
                return workflowService.createWorkflow(workflowReq, user.getCompany());
    ...
    private Workflow createWorkflow(WorkflowPostDTO workflowReq, Company company) {
        System.out.println("DEBUG: Controller createWorkflow called");
        System.out.println("DEBUG: Workflow title: " + workflowReq.getTitle());
        System.out.println("DEBUG: Secondary conditions count: " + (workflowReq.getSecondaryConditions() != null ? workflowReq.getSecondaryConditions().size() : 0));
        if (workflowReq.getSecondaryConditions() != null && !workflowReq.getSecondaryConditions().isEmpty()) {
            workflowReq.getSecondaryConditions().forEach(cond -> {
                System.out.println("DEBUG: Condition from DTO - alertName: " + cond.getAlertName() + ", severity: " + cond.getSeverity());
            });
        } else {
            System.out.println("DEBUG: No secondary conditions found");
        }
        
        List<WorkflowCondition> workflowConditions = workflowReq.getSecondaryConditions().stream().map(conditionDto -> {
            WorkflowCondition mapped = workflowConditionMapper.toModel(conditionDto);
            // Manually set alert fields if they're null (workaround for mapper issue)
            if (mapped.getAlertName() == null && conditionDto.getAlertName() != null) {
                mapped.setAlertName(conditionDto.getAlertName());
            }
            if (mapped.getSeverity() == null && conditionDto.getSeverity() != null) {
                mapped.setSeverity(conditionDto.getSeverity());
            }
            System.out.println("DEBUG: Mapped condition - alertName: " + mapped.getAlertName() + ", severity: " + mapped.getSeverity());
            return mapped;
