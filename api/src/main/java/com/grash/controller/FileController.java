...
public class FileController {
    ...
    public ResponseEntity<Page<FileShowDTO>> search(@RequestBody SearchCriteria searchCriteria,
        ...
        if (user.getRole().getRoleType().equals(RoleType.ROLE_CLIENT)) {
            if (user.getRole().getViewPermissions().contains(PermissionEntity.DOCUMENTS)) {
                ...
                boolean canViewOthers = user.getRole().getViewOtherPermissions().contains(PermissionEntity.DOCUMENTS);
                if (!canViewOthers) {
    ...
    public FileShowDTO patch(@ApiParam("File") @Valid @RequestBody FilePatchDTO file,
        ...
        if (optionalFile.isPresent()) {
            File savedFile = optionalFile.get();
            if (user.getRole().getEditOtherPermissions().contains(PermissionEntity.DOCUMENTS) || savedFile.getCreatedBy().equals(user.getId())) {
    ...
    public ResponseEntity<SuccessResponse> delete(@ApiParam("id") @PathVariable("id") Long id, HttpServletRequest req) {
        ...
        if (optionalFile.isPresent()) {
            ...
            if (user.getId().equals(savedFile.getCreatedBy())
                    || user.getRole().getDeleteOtherPermissions().contains(PermissionEntity.DOCUMENTS)) {
