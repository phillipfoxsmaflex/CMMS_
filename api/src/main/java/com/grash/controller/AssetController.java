...
public class AssetController {
    ...
    @ApiResponses(value = {//
            ...
            @ApiResponse(code = 403, message = "Access denied"),
            @ApiResponse(code = 404, message = "Asset not found")})
    public AssetShowDTO setAssetDashboard(
            @ApiParam("id") @PathVariable("id") Long id,
        ...
        if (optionalAsset.isPresent()) {
            ...
            if (user.getRole().getEditOtherPermissions().contains(PermissionEntity.ASSETS) || 
                ...
                Asset updatedAsset = assetService.setDashboardConfig(id, config.getUrl(), config.getConfig());
                return assetMapper.toShowDto(updatedAsset, assetService);
            } else throw new CustomException("Access denied", HttpStatus.FORBIDDEN);
        } else throw new CustomException("Asset not found", HttpStatus.NOT_FOUND);
    }
    ...
    @ApiResponses(value = {//
            ...
            @ApiResponse(code = 403, message = "Access denied"),
            @ApiResponse(code = 404, message = "Asset not found")})
    public DashboardConfigDTO getAssetDashboard(
            @ApiParam("id") @PathVariable("id") Long id,
            HttpServletRequest req) {
        OwnUser user = userService.whoami(req);
        Optional<Asset> optionalAsset = assetService.findById(id);
        if (optionalAsset.isPresent()) {
            Asset asset = optionalAsset.get();
            if (user.getRole().getViewPermissions().contains(PermissionEntity.ASSETS)) {
                java.util.Map<String, String> config = assetService.getDashboardConfig(id);
