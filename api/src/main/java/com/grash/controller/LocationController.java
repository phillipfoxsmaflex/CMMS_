...
public class LocationController {
    ...
    @ApiResponses(value = {//
            ...
            @ApiResponse(code = 403, message = "Access denied"),
            @ApiResponse(code = 404, message = "Location not found")})
    public LocationShowDTO setLocationDashboard(
            @ApiParam("id") @PathVariable("id") Long id,
        ...
        if (optionalLocation.isPresent()) {
            ...
            if (user.getCompany().getId().equals(location.getCompany().getId())) {
                Location savedLocation = locationService.setDashboardConfig(id, config.getUrl(), config.getConfig());
                return locationMapper.toShowDto(savedLocation, locationService);
            } else throw new CustomException("Access denied", HttpStatus.FORBIDDEN);
        } else throw new CustomException("Location not found", HttpStatus.NOT_FOUND);
    }
    ...
    @ApiResponses(value = {//
            ...
            @ApiResponse(code = 403, message = "Access denied"),
            @ApiResponse(code = 404, message = "Location not found")})
    public DashboardConfigDTO getLocationDashboard(
            @ApiParam("id") @PathVariable("id") Long id,
            HttpServletRequest req) {
        OwnUser user = userService.whoami(req);
        Optional<Location> optionalLocation = locationService.findById(id);
        if (optionalLocation.isPresent()) {
            ...
            if (user.getCompany().getId().equals(location.getCompany().getId())) {
                ...
                config.setUrl(location.getDashboardUrl());
                config.setConfig(location.getDashboardConfig());
                return config;
            } else throw new CustomException("Access denied", HttpStatus.FORBIDDEN);
        } else throw new CustomException("Location not found", HttpStatus.NOT_FOUND);
    }
