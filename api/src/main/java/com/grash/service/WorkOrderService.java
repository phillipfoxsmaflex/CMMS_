        ...
        if (!users.isEmpty()) {
            try {
                emailService2.sendMessageUsingThymeleafTemplate(
                    users.stream().map(OwnUser::getEmail).toArray(String[]::new),
                    messageSource.getMessage("new_wo", null, locale),
                    mailVariables,
                    "assigned-work-order.html",
                    Helper.getLocale(users.stream().findFirst().get())
                );
            } catch (Exception e) {
