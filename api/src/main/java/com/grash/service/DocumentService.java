...
public class DocumentService {
    ...
    public List<DocumentDTO> getAllDocuments(Long companyId) {
        log.info("[DEBUG] getAllDocuments called: companyId={}", companyId);
        
        // Get ALL active documents (not just root documents)
        // This allows frontend to build custom hierarchies based on entityType
        List<Document> allDocuments = documentRepository.findByIsActiveTrue();
        
        log.info("[DEBUG] Found {} total active documents", allDocuments.size());
        
        // Filter by company and convert to DTO (flat list, no tree structure)
        List<DocumentDTO> result = allDocuments.stream()
            .filter(doc -> doc.getCompany().getId().equals(companyId))
