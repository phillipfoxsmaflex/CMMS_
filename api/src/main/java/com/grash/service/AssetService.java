...
public class AssetService {
    ...
    public Asset create(Asset asset, OwnUser user) {
        ...
        applicationEventPublisher.publishEvent(new AssetCreatedEvent(savedAsset));
        return savedAsset;
    }
    ...
    public Asset updatePosition(Long id, AssetPositionDTO positionDTO) {
        ...
        if (optionalAsset.isPresent()) {
            ...
            if (positionDTO.getFloorPlanId() != null) {
                FloorPlan floorPlan = em.getReference(FloorPlan.class, positionDTO.getFloorPlanId());
                asset.setFloorPlan(floorPlan);
            } else {
                asset.setFloorPlan(null);
            }
            
            asset.setPositionX(positionDTO.getPositionX());
            asset.setPositionY(positionDTO.getPositionY());
    ...
    @Transactional
    public Asset setDashboardConfig(Long assetId, String url, String config) {
        Optional<Asset> optionalAsset = findById(assetId);
        if (optionalAsset.isPresent()) {
            Asset asset = optionalAsset.get();
            asset.setDashboardUrl(url);
            asset.setDashboardConfig(config);
            return assetRepository.save(asset);
        }
        throw new EntityNotFoundException("Asset not found with id: " + assetId);
    }

    public Map<String, String> getDashboardConfig(Long assetId) {
        Optional<Asset> optionalAsset = findById(assetId);
        if (optionalAsset.isPresent()) {
            Asset asset = optionalAsset.get();
            Map<String, String> config = new HashMap<>();
            config.put("url", asset.getDashboardUrl());
            config.put("config", asset.getDashboardConfig());
            return config;
        }
        throw new EntityNotFoundException("Asset not found with id: " + assetId);
    }
