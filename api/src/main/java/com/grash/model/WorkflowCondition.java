...
public class WorkflowCondition extends CompanyAudit {
    ...
    public boolean isMetForWebhook(GrafanaAlert alert) {
        // A condition is met if all its specified fields match
        // If a field is null/empty in the condition, it's not checked
        
        boolean alertNameMatches = true;
        boolean severityMatches = true;
        
        // Check alert name if this condition specifies it
        if (alertName != null && !alertName.isEmpty()) {
            alertNameMatches = alertName.equals(alert.getAlertName());
        }
        
        // Check severity if this condition specifies it
        if (severity != null && !severity.isEmpty()) {
            severityMatches = severity.equalsIgnoreCase(alert.getSeverity());
        }
        
        return alertNameMatches && severityMatches;
    }
